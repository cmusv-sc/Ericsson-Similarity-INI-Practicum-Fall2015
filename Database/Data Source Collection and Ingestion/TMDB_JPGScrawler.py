...
def main():
    ...
    for iteration in range(0, int(Mapping.count()['movielen_id'])):
        ...
        if incomplete_poster_URL is None:
            ERROR_CODE = "[ERROR] TMDB records not exist!!! " + str(movielens_id)
            print ERROR_CODE
            LOG = open('Poster Logging.txt', "a")
...
def Download_Poster(movielens_id, poster_URL, num_retries = 3):
    ...
    try:
        ...
        ERROR_CODE = "[ERROR] " + error.reason + " " + str(movielens_id)
        print ERROR_CODE
        LOG = open('Poster Logging.txt', "a")
        LOG.write(ERROR_CODE)
        LOG.write('\n')
